"""PRESENT: fast, default-run known-answer test on the generated Python (unrolled)
...
def test_present_permutation_kat():
    _kat(PRESENT_PERMUTATION(r=None))


@pytest.mark.parametrize("v", [[64, 80], [64, 128]])
def test_present_blockcipher_kat(v):
    _kat(PRESENT_BLOCKCIPHER(r=None, version=v))
