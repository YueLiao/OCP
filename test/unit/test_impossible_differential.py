...
from attacks import impossible_differential as impd


# ---------- goal validation (checked before touching the cipher, so no solver needed) ----------
def test_impossible_search_rejects_unknown_goal():
    with pytest.raises(ValueError, match="Invalid goal"):
        impd.search_id_distinguisher_enumeration(None, goal="NOPE")
...
def test_fixed_activity_constraints_word_level_milp():
    ...
    cons = impd._gen_fixed_activity_constraints(vs, active_positions=[0, 2], model_type="milp", bitwise=False)
    assert cons == ["a = 1", "b = 0", "c = 1"]


def test_fixed_activity_constraints_word_level_sat():
    vs = [var.Variable(1, ID="a"), var.Variable(1, ID="b")]
    cons = impd._gen_fixed_activity_constraints(vs, active_positions=[1], model_type="sat", bitwise=False)
    assert cons == ["-a", "b"]  # inactive -> -a, active -> b


def test_fixed_activity_constraints_bit_level():
    vs = [var.Variable(2, ID="a")]  # -> ids a_0, a_1
    cons = impd._gen_fixed_activity_constraints(vs, active_positions=[1], model_type="milp", bitwise=True)
...
def test_parse_and_set_configs_defaults_and_filename():
    ...
    cm, cs = impd._parse_and_set_configs(cipher, goal, {}, {})
    ...
    cm_sat, _ = impd._parse_and_set_configs(cipher, goal, {"model_type": "sat"}, {})
    assert cm_sat["filename"].endswith("sat_model.cnf")


# ---------- facade delegation (attacks.py wiring) ----------
def test_impossible_differential_attacks_delegates(monkeypatch):
    captured = {}

    def fake(cipher, goal, config_model, config_solver, show_mode):
        captured.update(cipher=cipher, goal=goal, show=show_mode)
        return [((0,), (1,))]

    monkeypatch.setattr(impd, "search_id_distinguisher_enumeration", fake)
    out = attacks.impossible_differential_attacks("CIPHER", goal="IMPOSSIBLEDIFF", show_mode=1)
    assert out == [((0,), (1,))]
    assert captured == {"cipher": "CIPHER", "goal": "IMPOSSIBLEDIFF", "show": 1}
