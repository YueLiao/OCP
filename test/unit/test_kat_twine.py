"""TWINE: fast, default-run known-answer test on the generated Python (unrolled)
...
def test_twine_permutation_kat():
    _kat(TWINE_PERMUTATION(r=None))


@pytest.mark.parametrize("v", [[64, 80], [64, 128]])
def test_twine_blockcipher_kat(v):
    _kat(TWINE_BLOCKCIPHER(r=None, version=v))
