"""Unit tests for solving/solving.py: solver capability discovery, name normalization,
availability checks, the MILP/SAT solve wrappers, and per-backend error handling
(Gurobi, SCIP, PySAT, OR-Tools CP-SAT) - with the actual solver backends mocked.
"""
import os
import subprocess
import sys
...
def test_solver_capabilities_are_queryable_without_importing_backends(monkeypatch):
    ...
    assert capabilities["sat"]["ORTools"]["implemented"] is True
...
def test_is_solver_available_respects_defaults_and_implemented_backends(monkeypatch):
    ...
    assert solving.is_solver_available("sat", "ORTools") is True
...
def test_cpsat_missing_backend_returns_none_with_warning(monkeypatch, capsys):
    # OR-Tools CP-SAT (the "ORTools" SAT backend) degrades to None + a warning when unavailable.
    monkeypatch.setattr(solving, "_load_ortools_cpsat", lambda: None)

    assert solving.solve_sat_cpsat("missing.cnf", {}, {}) is None
    assert "OR-Tools CP-SAT module can't be loaded" in capsys.readouterr().out
