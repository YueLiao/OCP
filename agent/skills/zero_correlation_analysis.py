...
class ZeroCorrelationAnalysisSkill(BaseSkill):
    """Enumerate zero-correlation linear distinguishers (truncated, miss-in-the-middle).

    Wraps attacks.zero_correlation_attacks. Returns the zero-correlation
    ``(input_positions, output_positions)`` truncated-linear-mask pairs.
    """

    @property
    def name(self) -> SkillName:
        return SkillName.ZERO_CORRELATION_ANALYSIS

    @property
    def description(self) -> str:
        return (
            "Search for zero-correlation linear distinguishers of the current cipher "
            "(truncated, miss-in-the-middle). Supports MILP and SAT. Returns the "
            "zero-correlation (input, output) active-word position pairs."
        )
    ...
    def execute(self, request: SkillRequest, session: Session) -> SkillResult:
        ...
        distinguishers, failure = run_analysis_attack(
            ...
            expected_error_prefix="Zero-correlation analysis failed",
            unexpected_error_prefix="Unexpected zero-correlation analysis failure",
            attack_fn=attacks.zero_correlation_attacks,
            cipher=cipher,
        ...
        return SkillResult(
            ...
            summary=f"Zero-correlation analysis ({model_type.upper()}): found {count} distinguisher(s).",
        )
